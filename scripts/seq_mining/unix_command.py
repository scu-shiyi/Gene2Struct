...
import scripts.seq_mining.build_trimed as build_trimed
import scripts.seq_mining.fix_alignment as fix_alignment
import scripts.seq_mining.muscle_wrapper as muscle_wrapper
...
def do_filter_assemble(args, samples, do_filter, do_assemble, ignore_hook=lambda *_, **__: None):
    ...
    if do_filter:
        ...
        def run_filter(name):
            ...
            #
            #     for scripts in genes:
            #         read_1 = os.path.join(out_dir, f'{scripts}_1{sample_ext}')
            #         read_2 = os.path.join(out_dir, f'{scripts}_2{sample_ext}')
            #
            #         if not os.path.isfile(read_1):
            #             continue
            #
            #         with open(os.path.join(merge_dir, scripts + sample_ext), 'wb') as f:
...
def build_coalescent_tree(args):
    ...
    if not genes:
        raise RuntimeError(f"No scripts alignments found under '{alignment_dir}'")
    ...
    if not written:
        raise RuntimeError(f"Unable to reconstruct coalescent trees because no scripts tree is available")
...
def cli(argv=None):
    parser = argparse.ArgumentParser(formatter_class=argparse.RawTextHelpFormatter,
                                     description='seq_mining is a tool for extracting phylogenetic marker genes.')
    ...
    parser.add_argument('-m', '--tree-method', choices=('coalescent', 'concatenation'), default='coalescent', help='Multi-scripts tree reconstruction method (default = coalescent)')
