...
#
# $Header: /mnt/raid/services/cvs/python/packages/share1.5/MolKit/listSet.py,v 1.32.4.1 2015/08/29 01:26:36 sanner Exp $
#
...
class ListSet(UserList):  # would (list) work???
    ...
    def __init__(self, data=None, elementType=None, stringRepr=None,
                 comments="", keywords=[]):
        # --- 在 __init__ 开头、任何 len(data) 之前加这段 ---
        if data is not None and not isinstance(data, (list, tuple)):
            # filter/map/dict_keys/生成器 等都在这里被物化
            try:
                data = list(data)
            except TypeError:
                # 某些对象不可迭代就保持原样，后面自然会暴露真正问题
                pass
        if data is not None and len(data)!=0:
            assert (hasattr(list(data)[0], '__class__'))
            if elementType is not None:
                assert (isinstance(list(data)[0], elementType))
        ...
        if member[:2]=='__':
            ...
            if not misc.issequence(value): # simple number of string
                ...
                if len(value) == 0: # empty sequence .. assign sequence to attribute
                    ...
                    for o, v in zip(self.data, value):
                        setattr(o, member, v)
...
#                        self.data[i].__dict__[member] = value[i]
    ...
    def inter(self, right):
        ...
        if self.stringRepr and right.stringRepr:
            ...
            stringRepr = None
        return self.__class__(misc.uniq(list(newlist)), stringRepr=stringRepr)
    ...
    def subtract(self, right):
        ...
        newlist = list(filter( lambda x: x._setFlag==1, self.data ))
        for o in self.data:
            if hasattr(o, '_setFlag'):
                del o._setFlag
        for o in right.data:
