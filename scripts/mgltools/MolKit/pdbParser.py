...
#
# $Header: /mnt/raid/services/cvs/python/packages/share1.5/MolKit/pdbParser.py,v 1.132.2.1 2015/08/29 01:26:36 sanner Exp $
#
...
Example:
    ...
    from scripts.mgltools.MolKit.protein import Protein
    from scripts.mgltools.MolKit.PdbParser import PdbParser
    mol = Protein()
...
# The original code imported helpers from the legacy `string` module:
#   from string import split, strip, digits, lower, find
# In Python 3 those callables are no longer exposed by `string`.
# We provide drop-in equivalents so the rest of this file can remain unchanged.

digits = string.digits

def split(s, sep=None, maxsplit=-1):
    """Drop-in replacement for legacy string.split."""
    if maxsplit is None or maxsplit == -1:
        return s.split(sep)
    return s.split(sep, maxsplit)

def strip(s):
    """Drop-in replacement for legacy string.strip."""
    return s.strip()

def lower(s):
    """Drop-in replacement for legacy string.lower."""
    return s.lower()

def find(s, sub, start=0, end=None):
    """Drop-in replacement for legacy string.find."""
    if end is None:
        return s.find(sub, start)
    return s.find(sub, start, end)
from scripts.mgltools.MolKit.moleculeParser import MoleculeParser
from scripts.mgltools.MolKit.protein import Protein, Chain, ChainSet, Residue, ResidueSet, ProteinSet
from scripts.mgltools.MolKit.molecule import Atom, AtomSet, Bond, BondSet, HydrogenBond
...
##          print 'FreeParser'
    ...
    def getKeys(self):
        if self.allLines:
            ...
            self.keys = list(map(lambda x: strip(x[:6]), self.allLines))
...
You have to provide one as the second argument' % key);
    ...
    def parse_PDB_MODRES(self, lines):
        ...
        for line in lines:
            ...
            from scripts.mgltools.MolKit.PDBresidueNames import DNAnames, RNAnames, AAnames,\
    ...
    def parse_PDB_atoms(self, atoms, mol):
        ...
        for atRec in atoms:
            ...
            try:
                ...
                #print("Error while parsing line %d in %s:\n" % (self.allLines.index(atRec), self.filename) + atRec)
                lineno = self.allLines.index(atRec)
                raise RuntimeError(
                    "Error while parsing line %d in %s: %s" % (lineno, self.filename, atRec)
                ) from inst
    ...
    def addModel(self, atoms):
        ...
        for at, crd in zip(self.mol.chains[0].residues.atoms, coords):
            Atom.addConformation(at, crd)
    ...
    def parse(self, objClass=Protein):
        ...
        pdbKeys = [k for k in self.keys if k in self.PDBtags]
        if len(pdbKeys) == 0:
            return
...
##                                                 self.pdbRecordParser[record])
    ...
    def getAtomsLines(self, ter, begChain):
        ...
        atoms = [x for x in self.allLines[begChain:ter+1] if l(x)]
        return atoms
...
##                                 line[71:76] ))
    ...
    def hasSsDataInFile(self):
        ...
        are in the file"""
        if any(x in self.keys for x in ['HELIX', 'SHEET', 'TURN']):
            return 1
        else:
            return 0
    ...
    def parseSSData(self, mol):
        ...
        """
        from scripts.mgltools.MolKit.protein import Helix, Strand, Turn
