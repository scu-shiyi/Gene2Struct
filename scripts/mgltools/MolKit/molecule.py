...
#
# $Header: /mnt/raid/services/cvs/python/packages/share1.5/MolKit/molecule.py,v 1.96.2.4 2016/02/20 01:32:12 annao Exp $
#
...
#import misc
from scripts.mgltools.mglutil.util import misc
...
##             raise RuntimeError("argument has to be a function or a string")
    ...
    def __getattr__(self, member):
        ...
        if member=='bonds':
            ...
            for b in flat:
                if l(b):
                    ...
                    b.atom2._inbnd = 1
            result = BondSet( res )
            atnobnd = AtomSet( list(filter( lambda x: x._inbnd == 0, uni ) ))
            for a in uni:
                if hasattr(a, '_inset'):
                    del a._inset
                if hasattr(a, '_inbnd'):
        ...
        elif len(result) and isinstance(result[0], dict):
    ...
    def addCharges(self, chargeSet, newCharges):
        ...
        for a, c in zip(self.data, newCharges):
...
from scripts.mgltools.PyBabel.babelElements import babel_elements
...
##             raise RuntimeError("argument has to be a function or a string")
    ...
    def findType(self, what, uniq=0):
        from needmokitMolKit.protein import Protein
...
class Molecule(TreeNode):
    ...
    """
    from scripts.mgltools.MolKit.radii_patterns import AtomRadiiPatterns, AAradii
    compiled_patterns = []
...
"""
    ...
    def write(self, filename, nodes, writer, sort=False, sortFunc=None,
        ...
        """
        from scripts.mgltools.MolKit.moleculeWriter import MoleculeWriter
