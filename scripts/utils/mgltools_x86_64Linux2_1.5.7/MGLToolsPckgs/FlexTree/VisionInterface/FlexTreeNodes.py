...
# ..../i86Linux2/lib/python2.3/site-packages/scipy/ga/examples.py
    ...
    settings = {'pop_size':popSize,
                'p_replace':p_replace,'p_cross': p_cross, 'p_mutate':'scripts',
...
##                 self.viewer.Redraw()
    ...
    class ConfigureTreeFromGene(NetworkNode):
        """Update the conformation with scripts.
...
it as an attribute of Atom 
    ...
    for i in range(len(ADScoreArray)):
        e = numpy.add.reduce(ADScoreArray[i])
        if i == 0:
...
"""
    ...
    if self.motion.node is not None:
        if type(matrixList) is types.ListType:
            matrixList = numpy.array(matrixList, 'f')
        if type(indexList) is types.ListType:
            indexList = numpy.array(indexList, 'f')        
...
4x4 transformation matrix """
    ...
    if RefAtomSet is not None and MobileAtomSet is not None and len(MobileAtomSet)==len(RefAtomSet):
        ...
        transMat = numpy.array(aligner.translationMatrix)
        rotMat[3,:3] = transMat     
...
"""
    ...
    #print oldMats
    import numpy
    newMat = numpy.array(in1[0])
    newMat = newMat.astype('f')
    newMat = numpy.transpose(newMat)
    newMat = numpy.reshape(newMat, (16,))

    mats=[]
    for m in oldMats:
        m=numpy.reshape(m, (4,4))
        mm= numpy.transpose(m)
        mm = numpy.reshape(mm, (16,))
        mats.append(mm)
    mats.append(newMat)
    #print len(mats)
...
"""
    ...
    tmp.reverse()
    result.extend(tmp)
    result = numpy.array(result,'f')/steps
