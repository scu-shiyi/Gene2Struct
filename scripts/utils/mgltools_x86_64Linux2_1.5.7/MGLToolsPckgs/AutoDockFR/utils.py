...
def getResultsFromLogFile(logFile):
    """locate the best scripts from a pyDock docking log file (pyDockLogFileName)
...
"""
    ...
    def getResidues(self, mol, resString, css):
        # we overwrite the molecule name with the mol.name
        if  resString.find(';') != -1:
            flexchains = resString.split(';')
            ww = flexchains[0]
        ...
        #w = resString.split(':')
        #if len(w)==3:
        #    resString = '%s:%s:%s'%(mol.name, w[1], w[2])

        result = css.select(MoleculeSet([mol]), resString)
        if len(result[0])==0:
            raise ValueError("ERROR: %s does not select anything in molecule %s"%(resString, mol.name))
        elif len(result[0])!=len(w[2].split(',')):
            raise ValueError("ERROR: %s selected %d residues while %d were expected"%(resString, len(result[0]), len(w[2].split(','))))

        result = result[0]                

        # make sure it's a ResidueSet
        if not isinstance(result, ResidueSet):
            raise ValueError("ERROR: %s does not select residues in molecule %s"%(resString, mol.name))

        return result

    
    def __init__(self, mol, movingSC, frozenSC=None):
        """
        ...
        ## handle movingSC keyword
        result = self.getResidues(mol, movingSC, css)

        ## handle frozenSC keyword
        if frozenSC is not None:
            resultF = self.getResidues(mol, frozenSC, css)
        ...
        for res in result: # loop over moving residues
            ...
            if frozenSC is not None:
                if res in resultF:
                    motion.deactivate(res.atoms[6:].coords)
...
##     """ This function generate PDB file for FlexTree configured by the scripts
## e.g.
...
##         TotalScore=scoreObject.score(gnm, scripts)
##         #print TotalScore
##         scoreObject.printAllScoreTerms()

##         if output !=None and output !="":
##             R_coords, L_coords = scripts.toPhenotype(sort=True)
##             rec.sort()  #?
...
##     for i in  range(len(geneList)):
##         scripts=geneList[i]
##         R_coords=gnm.toPhenotype(scripts, sort=True )
##         rec.updateCoords(R_coords, 1)
##         rec.setConformation(1)            
##         writer.write(output+str(i)+ext, rec)
##         print scripts
##         print -gnm.scorer.score(gnm, scripts)
...
##     for i in  range(len(geneList)):
##         scripts=geneList[i]
##         score= 0.0 - scoreObject.score(gnm, scripts)
##         #print "score =",score
##         #print scoreObject.printAllScoreTerms()
##         scores.append(score)
##         R_coords, L_coords=gnm.toPhenotype(scripts, sort=True)
