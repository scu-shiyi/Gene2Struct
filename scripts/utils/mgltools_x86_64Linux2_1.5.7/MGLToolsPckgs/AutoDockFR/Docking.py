...
#########################################################################
#
# $Header: /mnt/raid/services/cvs/AutoDockFR/Docking.py,v 1.39.2.1 2015/08/26 21:06:02 sanner Exp $
#
# $Id: Docking.py,v 1.39.2.1 2015/08/26 21:06:02 sanner Exp $
#
"""
...
if foundAutoDockC:
    ...
    class AutoDockFR:
        ...
        def dock(self, initialPopulation=None):
		    ...
		    RecRecEnergy = 0.0
                ...
                print 'FINAL SOLUTION: %3d FEB: %9.3f R-L: %9.3f L: %9.3f R-R: %9.3f wtRR: %9.3f Tor: %9.3f Score: %9.3f '%(
                    idx, ene, RecLigEnergy, InternalLigEnergy, RecRecEnergy, wtRREnergy, tor, -ind._fitness_score),
                ...
                comments.append('FINAL SOLUTION: %3d FEB: %9.3f R-L: %9.3f L: %9.3f R-R: %9.3f wtRR: %9.3f Tor: %9.3f Score: %9.3f'%(
                    idx, ene, RecLigEnergy, InternalLigEnergy, RecRecEnergy, wtRREnergy, tor, -ind._fitness_score))
    ...
    class Docking:
        ...
        def _prepareReceptor(self):
            ...
            if not setting.has_key("ReceptorXML"):
                ...
                if not setting.has_key("movingSC"):  ## a rigid receptor
                    ...
                    if setting.has_key("frozenSC"):
                        xmlbuilder = RecXML4SoftRotamSC(mol, setting["movingSC"], setting["frozenSC"])
                    else:
		...
		## # MLD: need a unique filename when runing repeat in the same folder
            ...
            ## handle 'fixedLigandRotationAndTranslation', 'fixedLigandConformation'
            ...
            for m in L_MotionObjs:
                if isinstance(m, GAFTMotion_RotationAboutAxis):
                    if self.setting['fixedLigandConformation']:
                        m.active = False
                elif isinstance(m, FTMotionCombiner):
                    if self.setting['fixedLigandRotationAndTranslation']:
                        m.active = False
                    ## if isinstance(subm, GAFTMotion_BoxTranslation):
                    ## for subm in m.motionList:
                    ##     if isinstance(subm, GAFTMotion_BoxTranslation):
                    ##         if self.setting['fixedLigandTranslation']:
                    ##             subm.active = False
                    ##     elif isinstance(subm, GAFTMotion_RotationAboutPointQuat):
            ...
            #import pdb
            #pdb.set_trace()
            ## MS this code use use the atom name which required unique ligand atom names
            ## we changed it to use atom coordinates April 2015 (after submitting paper)
            # ligand set might be resuffled compared to self.ligand.allAtoms
            # create a lookup using ._FTindex
            # self.ligandSet aoms have conformation 1 (which is changing with the FlexTree motions
            # conformation 0 are the original ligand coordinates that we use to identify
            # ligand atoms i.e. a._coords[0] rather than a.coords which gets a._coords[a.conformation]
            ligPos = [] 
            for a in self.ligandSet:
                ligPos.append('%.3f,%.3f,%.3f'%tuple(a._coords[0]))
            #print ligPos
            #ligandSetNames = self.ligandSet.name
            #assert len(numpy.unique(ligandSetNames))==len(ligandSetNames)
            for a in self.ligand.allAtoms:
                #a._FTindex = ligandSetNames.index(a.name)
                #print a.coords
                #import pdb
                #pdb.set_trace()
                a._FTindex = ligPos.index('%.3f,%.3f,%.3f'%tuple(a.coords))
	    ...
	    """
            ...
            return
            ## # Retreive the best scripts & its score
	    ...
	    ## #import pdb
	    ## #pdb.set_trace()
            ## # Transform the scripts into x,y,z coords
	    ...
	    ## #import pdb
	    ## #pdb.set_trace()
            ## # Check the score of the best scripts, print scoring breakdown
        ...
        def _setupSearch(self):
            ...
            for refMolName in self.setting['rmsdRecRef']:
                ...
                refMol.buildBondsByDistance()
                ###unbound RR test
                ubRecCoords = refMol.allAtoms.coords
		...
		from AutoDockFR.ADCscorer import AD42ScoreC
                ...
                if self.setting.has_key("movingSC"):
                    ...
                    ###unbound Rec-RecEnergy
                    fsc,sc = scoreObject.score(self.rigidRecAtoms.coords, ubRecCoords,self.ligandSet.coords,
                                               RR_L=False, FR_L=False, L_L=False, 
                                               RR_RR=False, RR_FR=True, FR_FR=True)
                    self.setting['RRUnbound'] = scoreObject.scoreBreakdown['RRFR']+scoreObject.scoreBreakdown['FRFR']
                    ###print fsc,sc, self.setting['RRUnbound'], scoreObject.scoreBreakdown['RRFR'], scoreObject.scoreBreakdown['FRFR']

                else:
                    from MolKit.molecule import AtomSet
                    self.flexRecAtoms = AtomSet([])
                    self.setting['RRUnbound'] = 0
            ...
            # a GA representation (template of a scripts)
            ...
            nLigGenes = 7+self.TORSDOF
            print "Number of Ligand genes:", nLigGenes

            if gscorer:
                gscorer.fixTranslation(self, fixMaps=setting['fixMaps'])
                if setting['fixMaps']:
                    raise
            ...
            if self.setting['GA_pop_size']=='auto':
                ...
                self.setting['GA_pop_size'] = (50 + 10*nLigGenes)
                ####self.setting['GA_pop_size'] = (50 + 10*gnm.totalGeneNum)
    ...
    class Dock_Serial(Docking):
        ...
        def validateScore(self, best):
            # Take the best scripts, create coords, score it
	    ##import pdb
	    ##pdb.set_trace()
    ...
    class Dock_DivideAndConquer(Dock_Serial):
        ...
        def configure(self, debug = 0):
            ...
            debug: debugging level
                    0: no debugging
                    1: output best scripts of each smaller boxes
                    2: ...
            """
